...
void get_candidate(bu_interval* v, const tVI& pattern_len, const tVI& min_occ,
                   const tVI& max_occ, vector<tVI>& candidates,
                   tVI& candidates_nr)
{

    for (size_t p = 0; p < pattern_len.size(); ++p) {
        if (v->lcp < pattern_len[p]) { // if depth of node is smaller than the
                                       // desired pattern_len
            for (size_t i = 0; i < v->children.size();
                 ++i) { // iterate over all children
                if (v->children[i]->lcp
                    >= pattern_len[p]) { // and check, if the depth is greater
                                         // or equal than pattern_len
                    size_t occ = v->children[i]->size();
                    if (min_occ[p] <= occ and occ
                        <= max_occ[p]) { // if frequency constraints are meet
                        ++candidates_nr[p];
                        if (candidates_nr[p]
                            < 1000000) { // limit candidates to ten million
                            candidates[p].push_back(
                                v->children[i]->lb); // add candidate
                        }
                    }
                }
            }
...
int main(int argc, char* argv[])
{
    ...
    if (argc < 4) {
        cout << "Usage: " << argv[0]
             << " lcp_file sa_file text_file [pattern_cnt]" << endl;
        cout << " output 25 files "
                "basename(text_file).<pattern_len>.<pattern_cnt>.0.<min_occ>.<"
                "max_occ>.pattern" << endl;
        return 1;
    }
    lcp_file = argv[1];
        ...
        if (pattern_stream) {
            ...
            if (cand.size() > 0) {
                ...
                if (text_stream) {
                    ...
                    for (size_t i = 0, r = 0, r_sum = 0;
                         i < sa_buf.int_vector_size and idx_sa_indexes
                         < sa_indexes.size();) {
                        for (; i < r + r_sum and idx_sa_indexes
                               < sa_indexes.size();
                             ++i) {
                            if (i == sa_indexes[idx_sa_indexes]) {
                                ...
                                pattern_stream.write(pattern_buf,
                                                     pattern_len[rr]);
                            }
                        }
                    ...
                    std::cerr << "ERROR: Could not open text file " << text_file
                              << endl;
                }
            }
            ...
            std::cerr << "ERROR: Could not open pattern file " << pattern_file
                      << endl;
        }
    }
}
