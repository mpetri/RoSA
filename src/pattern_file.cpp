...
pattern_file::pattern_file(const char* pattern_file_name)
    : pattern_cnt(m_pattern_cnt), pattern_len(m_pattern_len), swaps(m_swaps)
{
    m_buf = NULL;
...
void pattern_file::reset()
{
    ...
    if (m_pattern_stream.is_open()) {
        ...
        std::cerr << "ERROR: Could not open pattern file "
                  << m_pattern_file_name << endl;
    }
}
 ...
 * \param pattern_cnt			Number of pattern that should be
 *generated.
 * \param pattern_len			Length of each pattern that should be
 *generated.
 * \param swaps					The number of random neighbour
 *character swaps applied to the pattern.
 *
 * \par Details of the generation
 *      We generate a pattern by randomly jump to a position \f$i\f$ in the text
 *T and then
 *      we extract the substring \f$P=T[i..i+pattern_len-1]\f$. After the
 *extraction
 *      we chose \f$swaps\f$ times a random position \f$r\f$ in
 *[0..pattern_len-2] and swap characters
 *      \f$P[r]\f$ and \$fP[r+1]\f$.
 */
void pattern_file::generate(const char* text_file_name, size_type pattern_cnt,
                            size_type pattern_len, size_type swaps)
{
    ...
    if (pattern_stream) {
        ...
        if (text_stream) {
            ...
            std::cerr << "ERROR: Could not open text file " << text_file_name
                      << endl;
        }
...
const char* pattern_file::get_pattern() { return m_buf; }

void pattern_file::remove() { std::remove(m_pattern_file_name.c_str()); }

void pattern_file::generate_restricted(const char* lcp_file,
                                       const char* sa_file,
                                       const char* text_file_name,
                                       size_type pattern_cnt,
                                       size_type pattern_len, size_type min_occ,
                                       size_type max_occ)
{
        ...
        for (; i < r + r_sum; ++i) {
            if (i > 0) {
                ...
                while (lcp < stk.top()->lcp) {
                    stk.top()->rb = i - 1;
                    last_interval = stk.top();
                    stk.pop();
                    // process node
                    get_candidate(last_interval, pattern_len, min_occ, max_occ,
                                  candidates);
                    lb = last_interval->lb;
                    if (lcp <= stk.top()->lcp) {
    ...
    if (pattern_stream) {
        if (candidates.size() == 0) {
            m_pattern_cnt = 0;
            std::cerr << "Found no candidates for pattern_len = " << pattern_len
                      << std::endl;
            std::cerr << "min_occ = " << min_occ << " and max_occ = " << max_occ
                      << std::endl;
        } else if (candidates.size() < 100) {
            std::cerr << "Less then 100 candidates for pattern_len = "
                      << pattern_len << std::endl;
            std::cerr << "min_occ = " << min_occ << " and max_occ = " << max_occ
                      << std::endl;
        } else {
            std::cerr << candidates.size()
                      << " candidates for pattern_len = " << pattern_len
                      << std::endl;
            std::cerr << "min_occ = " << min_occ << " and max_occ = " << max_occ
                      << std::endl;
        }
        ...
        if (candidates.size() > 0) {
            ...
            if (text_stream) {
                ...
                for (size_type i = 0, r = 0, r_sum = 0;
                     i < sa_buf.int_vector_size and idx_sa_indexes
                     < sa_indexes.size();) {
                    for (; i < r + r_sum and idx_sa_indexes < sa_indexes.size();
                         ++i) {
                        if (i == sa_indexes[idx_sa_indexes]) {
                            //							std::cout<<"idx_sa_indexes="<<idx_sa_indexes<<"
                            //sa_indexes["<<idx_sa_indexes<<"]="<<sa_indexes[idx_sa_indexes]<<std::endl;
                            ++idx_sa_indexes;
                            //							std::cout<<idx_sa_indexes<<"
                            //sa_indexes.size()"<<sa_indexes.size()<<endl;
                ...
                std::cerr << "ERROR: Could not open text file "
                          << text_file_name << endl;
            }
        }
        pattern_stream.close();
...
void pattern_file::get_candidate(bu_interval* v, size_t pattern_len,
                                 size_t min_occ, size_t max_occ,
                                 std::vector<size_t>& candidates)
{
    if (v->lcp < pattern_len) { // if depth of node is smaller than the desired
                                // pattern_len
        for (size_t i = 0; i < v->children.size();
             ++i) { // iterate over all children
            if (v->children[i]->lcp
                >= pattern_len) { // and check, if the depth is greater or equal
                                  // than pattern_len
                size_type occ = v->children[i]->size();
                if (min_occ <= occ and occ
                    <= max_occ) { // if frequency constraints are meet
