...
public:
    ...
    pattern_file_ng(const char* pattern_file_name)
    {
        ...
        if (!ifs.is_open()) {
            std::cerr << "pattern file " << pattern_file_name
    ...
    void add_pattern(std::string P) { m_patterns.push_back(P); }
    ...
    size_type size() { return m_patterns.size(); }

    auto begin() -> decltype(m_patterns.begin()) { return m_patterns.begin(); }

    auto end() -> decltype(m_patterns.end()) { return m_patterns.end(); }
};
