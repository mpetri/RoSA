...
struct factor_matcher_helper {
	...
	static void print_pattern_alignment(const t_itr& start,const t_itr& it,const std::vector<uint8_t>& P)
	{
		auto dist = std::distance(start,it);
		std::cout << " P = ";
		for(size_t i=0;i<dist;i++) std::cout << "#";
		for(size_t i=0;i<P.size();i++) {
			if( isprint(P[i])) std::cout << P[i];
			else std::cout << "?";
		}
		std::cout << "'" << std::endl;
	}
	...
	static void print_sa(const sdsl::int_vector<32>& sa,const std::vector<uint8_t>& T) {
		for(size_t i=0;i<sa.size();i++) {
			std::cout << std::setw(3) << i << "  " << std::setw(3) << sa[i] << "  ";
			for(size_t j=sa[i];j<T.size();j++) {
				if( isprint(T[j])) std::cout << T[j];
				else std::cout << "_";
			}
			std::cout << "$\n";
		}
	...
	static size_t 
	search_sa(const sdsl::int_vector<32>& sa,const std::vector<uint8_t>& T,
		...
		while(matched < P.size()) {
			auto range = std::equal_range(sa.begin()+sp,sa.begin()+ep+1,P[pos],sa_cmp(T,matched));
			if( range.first != sa.begin()+ep+1 && std::distance(range.first,range.second) > 0 ) {
				matched++;
				sp = std::distance(sa.begin(),range.first);
				ep = std::distance(sa.begin(),range.second)-1;

				// prefix check
				auto prefix = std::find(range.first,range.second,sa.size()-matched);
		...
		if(matched == P.size()) {
			...
			if((ep+1-sp) > 1) {
				...
				size_t revpos_of_factor = T.size() - cur_alignment -1 - (P.size() - 1);
				bool aligned = true;
				for(size_t i=0;i<P.size();i++) {
					if(T[revpos_of_factor+i] != P[P.size()-1-i]) aligned = false;
				}
				if(aligned) {
...
struct factor_matcher_kmp {
	...
	static sdsl::int_vector<> calculate_kmp_shift_table(const std::vector<uint8_t>& P) {
		size_t m = P.size();
...
struct factor_matcher_bmh {
	...
	static sdsl::int_vector<> calculate_bmh_shift_table(const std::vector<uint8_t>& P) {
		size_t m = P.size();
...
struct factor_matcher_sa_smart {
	...
	template<class t_itr>
	static std::tuple<std::vector<uint8_t>,size_t,bool> determine_factor(t_itr it,const std::vector<uint8_t>& P) {
									...
									const std::vector<uint8_t>& P)
	{
		...
		auto shift = factor_matcher_helper::search_sa(sa,PRev,factor,factor_pos);
...
struct factor_matcher_mbmh_cd {
	...
	static std::vector<uint64_t> calculate_mbmh_shift_matrix(const std::vector<uint8_t>& P) {
	    auto m = P.size();
	...
	static void
	print_matrix(const std::vector<uint64_t>& M,const std::vector<uint8_t>& P)
	{
		size_t m = P.size();
	    ...
	    for(size_t i=0;i<exists.size();i++) {
	        if(exists[i] != 0) {
	        	if(isprint((int)i)) std::cout << (char)i << " ";
	            else std::cout << "? ";
	        }
	    }
	    std::cout << std::endl;
	    for(size_t i=0;i<m;i++) {
	        for(size_t j=0;j<exists.size();j++) {
	            if(exists[j] != 0) {
	                if(M[i*256+j] != 0) std::cout << M[i*256+j] << " ";
	                else std::cout << "  ";
	            }
	        }
	        std::cout << std::endl;
	...
	template<class t_itr>
	static t_itr find_new_alignment(t_itr it,const std::vector<uint64_t>& M,const std::vector<uint8_t>& P)
	{
		auto itr = it;
		size_t max_shift = 1;
		size_t m = P.size();
