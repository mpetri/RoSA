...
struct block_info {
    ...
    block_info(size_type bwd_lb, size_type depth, size_type fwd_lb,
               size_type size);
    bool operator<(const struct block_info& b) const; // sort according to
                                                      // bwd_lb, depth, fwd_lb
                                                      // and size
};
...
struct block_node {
    size_type delta_x;    // Start position in in the corresponding irreducible
                          // block.
    size_type delta_d;    // Number of iterations to reach the corresponding
                          // irreducible block.
    ...
    block_node(size_type delta_x = 0, size_type delta_d = 0,
               size_type dest_block = 0, size_type bwd_id = 0);
};

//! bwd_block_info holds information about the  Stores information of blocks in
//the backward index
struct bwd_block_info {
    ...
    bwd_block_info(size_type bwd_lb = 0, size_type bwd_rb = 0,
                   size_type depth = 0, size_type block_id = 0);
...
struct header_item {
    ...
    header_item(size_type bwd_id = 0, size_type delta_x = 0,
                size_type delta_d = 0);
