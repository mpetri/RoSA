...
public:
    ...
    //! Constructor takes the file name where the pattern should be written to
    //or read from
    pattern_file(const char* pattern_file_name);

    //! Opens the pattern file; after the call to rest we get the next pattern
    //be calling get_next_pattern
    /*!
     * \sa get_next_pattern
     */
    void reset();

    //! Generates a number of patterns from a given text and store them in a
    //file
    /*
     ...
     *generated.
     * \param swaps					The number of random neighbour character swaps
     *applied to the pattern.
     *
     * \par Details of the generation
     *      We generate a pattern by randomly jump to a position \f$i\f$ in the
     *text T and then
     ...
     */
    void generate(const char* text_file_name, size_type pattern_cnt,
                  size_type pattern_len, size_type swaps = 0);

    //! Generates a number of patterns with some restrictions (length and
    //occurrences)
    /*
     * \param cst 					Compressed suffix tree of the
     * text.
     * \param text_file_name 		File name of the text.
     * \param pattern_cnt			Number of pattern that should be
     * generated.
     * \param pattern_len			Length of each pattern that should be
     * generated.
     * \param min_occ				Minimum number of occurrences of the
     * pattern.
     * \param max_occ				Maximum number of occurrences of the
     * pattern.
     */
    template <class tCst>
    void generate_restricted(const tCst& cst, const char* text_file_name,
                             size_type pattern_cnt, size_type pattern_len,
                             size_type min_occ, size_type max_occ);

    void generate_restricted(const char* lcp_file, const char* sa_file,
                             const char* text_file_name, size_type pattern_cnt,
    ...
    void get_candidate(bu_interval* v, size_t pattern_len, size_t min_occ,
                       size_t max_occ, std::vector<size_t>& candidates);
...
template <class tCst>
void
pattern_file::generate_restricted(const tCst& cst, const char* text_file_name,
                                  size_type pattern_cnt, size_type pattern_len,
                                  size_type min_occ, size_type max_occ)
{
    ...
    for (typename tCst::const_iterator it = cst.begin(), end = cst.end();
        ...
        if (it.visit() == 1) {
            node_type v = *it;
            if (cst.depth(v)
                >= pattern_len) { // if the depth >= pattern_len we determine
