...
//! A wrapper class for a compressed suffix array that provides the same
//interface as the rosa class
/*
...
public:
     ...
     */
    in_memory_index(const char* file_name = NULL, size_type b = 4096,
                    size_type f_fac_dens = 1, bool output_tikz = false,
                    bool delete_tmp = false, const char* tmp_file_dir = "./",
                    const char* output_dir = NULL)
        : k(m_k), lz_width(m_lz_width), file_name(m_file_name),
          ...
          count_block_length(m_count_block_length), tmp_cst_suffix("")
    {
        m_k = 0;
        m_lz_width = 0;
        ...
        std::string tmp_dir = (util::dirname(tmp_file_dir) + "/"
    ...
    static string get_int_idx_filename(const char* file_name, size_type b,
                                       size_type fac_dens = 0,
                                       const char* output_dir = NULL)
    {
        return get_output_dir(file_name, output_dir) + "/"
               + util::basename(file_name) + "." + INDEX_SUF + ".int_idx";
    }

    string get_ext_idx_filename() { return get_int_idx_filename(); }
    ...
    double get_ext_idx_size_in_mega_byte() { return 0.0; }
    ...
    string get_factorization_filename() const { return ""; }
    ...
    {
        // count(): search for the reversed pattern, since we use the bwt of the
        // reversed string
                     ...
                     vector<size_type>& res) const
    {
        ...
        if (rb > lb + 1) {
            ...
            for (size_type i = lb; i <= rb; ++i) {
                res[i - lb] = m_csa[i];
#ifdef OUTPUT_STATS
                m_count_disk_access += m_csa.sa_sample_dens; // a potential disk
                                                             // access for every
                                                             // step
#endif
            }
                      ...
                      size_type& rb, size_type& d) const
    {
        ...
        while (d<m and rb - lb + 1> m_b) {
            ...
            algorithm::backward_search(m_csa, lb, rb, *cp, lb2, rb2);
            if (lb2 == rb2
                       + 1) { // the pattern does not exist in the pruned BWT
                return false;
            }
