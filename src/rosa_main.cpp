...
typedef in_memory_index<csa_wt<wt_huff<tRRR>, 16, 64000,
                               text_order_sa_sampling<sd_vector<>>>> tIDX;
#endif
...
#include <sdsl/rrr_vector.hpp>
typedef rrr_vector<63> tRRR;
typedef in_memory_index<csa_wt<wt_huff<tRRR>, 64, 64000,
                               text_order_sa_sampling<sd_vector<>>>> tIDX;
#endif
...
typedef in_memory_index<csa_wt<wt_huff<tRRR, tRRR::rank_1_type,
                                       tRRR::select_1_type,
                                       tRRR::select_0_type>,
                               64000, 64000,
                               text_order_sa_sampling<sd_vector<>>>> tIDX;
...
#include "in_memory_index.hpp"
typedef in_memory_index<csa_wt<wt_rlmn<sd_vector<>, sd_vector<>::rank_1_type,
...
void generate_index(tIndex& idx, const char* file_name, size_type b,
                    size_type fac_dens, bool output_tikz = false,
                    ...
                    const char* output_dir = ".")
{
    ...
    string ext_idx_file_name
        = tIndex::get_ext_idx_filename(file_name, b, fac_dens, output_dir);
    string factorization_file_name
        = tIndex::get_factorization_filename(file_name, b, output_dir);
    ...
    if (!int_idx_stream or !ext_idx_stream or(fac_dens
                                              > 0 and !factor_stream)) {
        if (!int_idx_stream) {
            cerr << "internal index is not stored in " << int_idx_file_name
                 << endl;
        }
        if (!ext_idx_stream) {
            cerr << "external index is not stored in " << ext_idx_file_name
                 << endl;
        }
        if (fac_dens > 0 and !factor_stream) {
            cerr << "factorization is not stored in " << factorization_file_name
                 << endl;
        }
        tIndex index(file_name, b, fac_dens, output_tikz, delete_tmp,
...
template <class tIndex>
void delete_index_files(const char* file_name, const char* output_dir,
                        size_type b, size_type fac_dens)
{
    string int_idx_file_name
        = tIndex::get_int_idx_filename(file_name, b, fac_dens, output_dir);
    string ext_idx_file_name
...
template <class tIndex>
void benchmark_matchlz(const char* file_name, size_type b, size_type fac_dens,
                       ...
                       bool locate_queries = false)
{
    ...
    for (const auto& P : pf) {
        ...
        if (index.matchlz_performed()) {
            ...
            std::cout << i << "-" << P.size() << "-" << count
                      << ";" // uniq id for qry
                      << base_name << ";" << b << ";" << fac_dens << ";" << i
                      << ";" << P.size() << ";" << matchlz_stats.method << ";"
                      << count << ";" << matchlz_stats.len_total << ";"
                      << matchlz_stats.len_accessed << ";"
                      << matchlz_stats.decoded << ";"
                      << matchlz_stats.num_factors << ";"
...
void benchmark(const char* file_name, size_type b, size_type fac_dens,
               const char* pattern_file_name, const char* tmp_file_dir,
               const char* output_dir, bool repeated_in_memory_search = false,
               bool only_in_memory = false, bool only_external_memory = false,
               bool locate_queries = false)
{

    cout << "# index_suf = " << INDEX_SUF << std::endl;
    cout << "# repeated_in_memory_search = " << repeated_in_memory_search
         << std::endl;
    ...
    if (!only_external_memory) {
        ...
        for (size_type i = 0; i < pf.pattern_cnt; ++i) {
            const unsigned char* c
                = (const unsigned char*)pf.get_next_pattern();
    ...
    if (!only_in_memory) {
        ...
        if (locate_queries) {
            for (size_type i = 0, count = 0; i < pf.pattern_cnt; ++i) {
                ...
                if (count == 0) {
                    cout << "i=" << i << endl;
                    std::cerr << "pattern_len = " << pf.pattern_len
                              << std::endl;
        ...
        } else // else do count queries
        {
            for (size_type i = 0, count = 0; i < pf.pattern_cnt; ++i) {
                const unsigned char* pat
                    = (const unsigned char*)pf.get_next_pattern();
                count = index.count(pat, pf.pattern_len,
                                    repeated_in_memory_search);
                cnt4 += count;
                cnt5 += (count > 0);
                if (count == 0) {
        ...
        if (pf.pattern_cnt > 0) {
            disk_access_per_query = index.count_disk_access
                                    / (double)pf.pattern_cnt;
            gap_disk_access_per_query = index.count_gap_disk_access
                                        / (double)pf.pattern_cnt;
            avg_int_match_depth = index.count_int_steps
                                  / (double)pf.pattern_cnt;
        }
        cout << "# disk_access_per_query = " << disk_access_per_query << endl;
        cout << "# gap_disk_access_per_query = " << gap_disk_access_per_query
             << endl;
        cout << "# avg_int_match_depth = " << avg_int_match_depth << endl;
        if (index.count_queries) {
            ratio_of_int_matched_queries = index.count_int_match
                                           / (double)index.count_queries;
        }
        cout << "# ratio_of_int_matched_queries = "
             << ratio_of_int_matched_queries << endl;
        size_type ext_queries = index.count_queries - index.count_int_match;
        double block_length_per_ext_query = 0;
        if (ext_queries) {
            block_length_per_ext_query = index.count_block_length
                                         / (double)ext_queries;
        }
        cout << "# block_length_per_ext_query = " << block_length_per_ext_query
             << endl;
#endif
    }
}
...
void display_usage(char* command)
{
    ...
    cout << " fac_dens         : sampling parameter for the factors; "
            "default=1; 0=don't use factorization" << endl;
    cout << " generate_index   : generates the index" << endl;
    cout << " generate_patterns: generate a pattern file; default=OFF" << endl;
    cout << " pattern_len      : length of each generated pattern; default=20"
         << endl;
    cout << " pattern_number   : number of generated patterns; default=100"
         << endl;
    cout << " pattern_swaps    : number of swaps applied to the patterns; "
            "default=0" << endl;
    cout << " pattern_min_occ  : minimum number of occurrences of a pattern in "
            "the text*; default=0" << endl;
    cout << " pattern_max_occ  : maximum number of occurrences of a pattern in "
            "the text*; default=0" << endl;
    ...
    cout << " benchmark_ext    : run benchmark for external memory part only; "
            "default=0" << endl;
    cout
        << " benchmark_loc    : run benchmark for locating queries; default = 0"
        << endl;
    cout << " pattern_file     : the pattern file for the benchmark" << endl;
    cout << " output_mem_info  : output information about the memory usage of "
            "the index" << endl;
    cout << " output_statistics: output statistics about the data structure"
         << endl;
    cout << " output_tikz      : output tikz code in the files: "
            "output_dir/basename(input_file).[f|b]wd_idx.tex" << endl;
    cout << " delete_tmp_file  : delete the temporary files after the "
            "construction; default=0" << endl;
    cout << " tmp_file_dir     : directory for the temporary files; default=./"
         << endl;
    cout << " output_dir       : directory for the constructed indexes; "
            "default=dirname(input_file)" << endl;
    cout << " delete_index_file: delete the generated index files; default=0"
         << endl;
    cout << " verbose          : activate verbose mode." << endl;
    cout << " interactive      : enter interactive mode after creating/loading "
            "the index." << endl;
    cout << " greedy           : do a greedy parse." << endl;
    cout << " reconstruct_text : reconstruct text from factorization and "
            "condensed BWT." << endl;
    cout << " factor_occ_freq  : outputs for all x the number of factors which "
            "occur x times in. Written to <input_file>.occ_freq" << endl;
    cout << " output_factors   : outputs all potential factors in file "
            "<input_file>.all_factors." << endl;
    cout << " add_factor_border: add information to the in-memory data "
            "structure to answer locate query for fac_dens>0" << endl;
    cout << endl;
    cout << "* if pattern_min_occ=pattern_max_occ=0 this restriction is not "
            "used in the " << endl;
...
int main(int argc, char* argv[])
{
    ...
    while (1) {
        static struct option long_options[]
            = {{"input_file", required_argument, 0, 'i'},
               ...
               {"repeated_in_memory_search", no_argument,
                &repeated_in_memory_search, 1},
    ...
    if (0 == fac_dens) {
        size_type input_size
            = (size_type)util::get_file_size(input_file_name.c_str());
        if (b > input_size) {
            std::cerr << "ERROR: input size (" << input_size
                      << ") is smaller than the block threshold b=" << b
                      << endl;
            return 1;
        }
    }
    ...
    if (do_generate_index) {
        tIDX index;
        generate_index(index, input_file_name.c_str(), b, fac_dens, false,
                       delete_tmp_file, tmp_file_dir.c_str(),
                       output_dir.c_str());
        string int_idx_file_name = tIDX::get_int_idx_filename(
            input_file_name.c_str(), b, fac_dens, output_dir.c_str());
        std::cerr << "load index from file " << int_idx_file_name << std::endl;
        util::load_from_file(index, int_idx_file_name.c_str());
        return 0;
    }
    ...
    if (interactive) {
        ...
        if (util::load_from_file(index, int_idx_file_name.c_str())) {
            std::cout << "Entering interactive mode. Please enter patterns or "
                         "Ctrl-D to exit." << std::endl;
            std::string s;
            int mode = 0;
            std::cout << "Enter \"l\" for location mode and press any other "
                         "key for counting mode." << std::endl;
            ...
            while (std::getline(cin, s)) {
                if (0 == mode) {
                    size_type cnt = index.count((const unsigned char*)s.c_str(),
                                                s.size(), false);
                    std::cout << ">>>>> pattern occurs " << cnt << " times"
                              << std::endl;
                } else {
                    std::vector<size_type> res;
                    size_type cnt = index.locate(
                        (const unsigned char*)s.c_str(), s.size(), res);
                    std::cout << ">>>>> pattern occurs " << cnt << " times"
                              << std::endl;
                    std::cout << "locations:";
    ...
    if (output_mem_info or output_statistics or greedy or reconstruct_text or
            factor_occ_freq or output_tikz or output_factors or
                add_factor_border) {
        tIDX index;
        string int_idx_file_name = tIDX::get_int_idx_filename(
            input_file_name.c_str(), b, fac_dens, output_dir.c_str());
        if (util::load_from_file(index, int_idx_file_name.c_str())) {
            ...
            if (output_mem_info) {
                ...
                size_type factors = index.greedy_parse(
                    tmp_file_dir, factor_borders, fac_dens);
                std::cout << "factors = " << factors << std::endl;
                std::cout << "avg factor length = " << ((double)index.size())
                                                       / factors << std::endl;
                std::cout << "k=" << index.k - 1 << std::endl;
                int bpf = bit_magic::l1BP(index.k - 1) + 1;
                std::cout << "bit_magic::l1BP(index.k-1)+1=" << bpf
                          << std::endl;
                std::cout << "lz_width=" << (int)index.lz_width << std::endl;
                std::cout << "factorization size in MB:"
                          << ((double)factors * bpf) / (8 * (1 << 20))
                          << std::endl;
            ...
            std::cerr << "ERROR: could not open file " << int_idx_file_name
                      << endl;
            return 1;
        }
        return 0;
    ...
    if (generate_patterns) {
        if (pattern_file_name == "") {
            std::string path
                = get_output_dir(input_file_name.c_str(), output_dir.c_str());
            pattern_file_name
                = path + "/" + util::basename(input_file_name) + "."
                  + util::to_string(plen) + "." + util::to_string(pno) + "."
                  + util::to_string(swapno) + "." + util::to_string(pmin_occ)
                  + "." + util::to_string(pmax_occ) + ".pattern";
        }
        cout << "-- start pattern generation -- " << endl;
        if (pmin_occ == pmax_occ and pmin_occ
            == 0) { // if the occurrence restriction is not initialized
            pattern_file pf(pattern_file_name.c_str());
            pf.generate(input_file_name.c_str(), pno, plen, swapno);
        } else {
            std::string path
                = get_output_dir(input_file_name.c_str(), output_dir.c_str())
                  + "/" + util::basename(input_file_name.c_str());
            ...
            cache_config config(false, tmp_dir,
                                util::basename(input_file_name));
            ...
            pf.generate_restricted(lcp_file.c_str(), sa_file.c_str(),
                                   input_file_name.c_str(), pno, plen, pmin_occ,
                                   pmax_occ);
            cout << "--" << endl;
        }
    ...
    if (run_benchmark or run_benchmark_int or run_benchmark_ext or
            locate_queries) {
        if (pattern_file_name == "") {
            std::string path
                = get_output_dir(input_file_name.c_str(), output_dir.c_str());
        ...
        benchmark<tIDX>(input_file_name.c_str(), b, fac_dens,
                        pattern_file_name.c_str(), tmp_file_dir.c_str(),
                        output_dir.c_str(), repeated_in_memory_search,
                        run_benchmark_int, run_benchmark_ext, locate_queries);
    }

    if (run_benchmark_matchlz) {
