...
template<class tIndex>
void benchmark_matchlz(const char* file_name, size_type b, size_type fac_dens, const char* pattern_file_name, const char* tmp_file_dir, const char* output_dir,
...
{
   {
    ...
    /* get patterns */
    //std::cerr << "reading patterns from file " << pattern_file_name << " ..." << std::endl;
    ...
    for (const auto& P : pf) {
        ...
        uint64_t count = index.count((const unsigned char*)P.c_str(), P.size(), repeated_in_memory_search);
        ...
        if(count == 0) {
            ...
            std::cerr << "'" << std::endl;
            std::cerr << i << " ERROR!. count is 0. should not happen. bug in code?" << std::endl;*/
            zero_cnt++;
            //exit(EXIT_FAILURE);
        }
        ...
        if(index.matchlz_performed()) {
            ...
            //static std::string filename = std::string("factor_lens")+"_"+std::to_string(b)+"_"+std::to_string(fac_dens)+".csv";
            ...
            std::cout << i << "-" << P.size() << "-" << count << ";" // uniq id for qry
                      << base_name << ";"
                      << b << ";"
                      << fac_dens << ";"
                      << i << ";"
                      << P.size() << ";"
                      << matchlz_stats.method << ";"
                      << count << ";"
                      << matchlz_stats.len_total << ";"
    ...
    std::cerr << "NP=" << pf.size() << " B=" << b << " R=" << fac_dens << " " << method << " ZC=" << zero_cnt << " CHECKSUM=" << checksum << std::endl;
}
...
int main(int argc, char* argv[])
{
    ...
    if(run_benchmark_matchlz) {
        ...
        //std::cerr << "pattern_file_name " << pattern_file_name << std::endl;
        benchmark_matchlz<tIDX>(input_file_name.c_str(), b, fac_dens, pattern_file_name.c_str(), tmp_file_dir.c_str(),
                        output_dir.c_str(), repeated_in_memory_search,
                        run_benchmark_int, run_benchmark_ext, locate_queries);
    }
}
