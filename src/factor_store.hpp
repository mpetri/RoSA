...
public:
    factor_iterator(store_type& s, size_t pos) : m_store_ptr(&s), m_cur_pos(pos)
    {
    ...
    bool operator==(const factor_iterator& b) const
    {
        return ((*this).m_cur_pos == b.m_cur_pos)
               && ((*this).m_store_ptr == b.m_store_ptr);
    }
    bool operator!=(const factor_iterator& b) const { return !(*this == b); }
    factor_iterator& operator+=(difference_type i)
    {
        m_cur_pos += i;
        m_cur_factor_id
            = m_store_ptr->pos_to_factor(m_cur_pos, m_cur_factor_id);
        return *this;
    }
    ...
    size_type cur_factor() const { return m_cur_factor_id; }
    size_type cur_offset() const { return m_cur_pos; }
    ...
    {
        auto next_decoded
            = m_store_ptr->next_decoded_pos(m_cur_pos, m_cur_factor_id);
...
private:
    size_type decode_factor_id(size_type i)
    {
        if (m_factors[i].len == 0) {
            ...
            if (i == 0) {
                ...
                m_factors[i].cumlen = m_factors[i - 1].cumlen
                                      + m_factors[i - 1].len;
            }
        }
    ...
    void decode_syms(size_type pos, size_type factor_id)
    {
        ...
        f.cur_lb = m_idx.decode_syms(f.cur_lb, items_to_decode,
                                     f.content.rbegin() + f.num_decoded);
        f.num_decoded += items_to_decode;
    }

    std::pair<size_type, size_type> next_decoded_pos(size_type pos,
                                                     size_type factor_id) const
    {
        size_t factor_offset = m_factors[factor_id].cumlen;
        size_t in_factor_offset = pos - factor_offset;
        size_t decoded_offset = m_factors[factor_id].len
                                - m_factors[factor_id].num_decoded;
        if (in_factor_offset < decoded_offset) {
            // skip to decoded in block
            return {factor_offset + decoded_offset, factor_id};
        }
        if (in_factor_offset != m_factors[factor_id].len - 1
            && in_factor_offset >= decoded_offset) {
        ...
        if (factor_id + 1 < m_factors.size() && m_factors[factor_id + 1].len) {
            ...
            decoded_offset = m_factors[factor_id + 1].len
                             - m_factors[factor_id + 1].num_decoded;
...
public:
    ...
    void print_factor(size_t id)
    {
        ...
        if (m_factors[id].num_decoded) {
            std::cout << ",syms = '";
            for (size_t i = 0;
                 i < m_factors[id].len - m_factors[id].num_decoded; i++) {
                std::cout << "_";
            }
            for (size_t i = m_factors[id].len - m_factors[id].num_decoded;
                 i < m_factors[id].len; i++) {
                if (isprint(m_factors[id].content[i]))
                    std::cout << m_factors[id].content[i];
                else
                    std::cout << "?";
            }
            std::cout << "'";
    ...
    size_type last_regular_factor() { return m_idx.fac_dens + 1; }

    size_type cum_decoded_len() const
    {
        return std::accumulate(
            m_factors.begin(), m_factors.end(), 0,
            [](size_t sum, const factor_state& f) { return sum += f.len; });
    }

    size_type num_decoded() const
    {
        return std::accumulate(m_factors.begin(), m_factors.end(), 0,
                               [](size_t sum, const factor_state& f) {
            return sum += f.num_decoded;
        });
    }

    size_type num_factors() const { return m_factors.size(); }
