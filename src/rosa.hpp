...
template <class BitVector, class RankSupport, class SelectSupport,
          class WaveletTree, class LcpSerializeWrapper, class LcpLoadWrapper>
class rosa {
public:
    ...
    //        typedef cst_sada<csa_wt<wt_rlmn<>,8, 256>, lcp_support_tree2<8> >
    // tCst;
...
private:
    mutable bool m_matchlz_performed;
    ...
    bit_vector_type m_bl; // indicates the BWT entries which corresponds to a
    ...
    bit_vector m_bm; // bit sequence to map from backward intervals to blocks in
                     // the external part
    ...
    size_type m_fac_dens; // every `m_fac_dens`-th factor is an entry point to
                          // the text
    ...
    mutable matchlz_stats m_mlz_stats;
    ...
    unsigned char* m_buf; // buffer for the text read from disk to match against
                          // the pattern
    ...
    mutable size_type m_count_gap_disk_access; // see corresponding public
                                               // member
    ...
    class disk_block {
        ...
        uint8_t m_width_bwd_id; // Bit width of the maximal value of a backward
                                // id in the header.
        uint8_t m_width_delta_x; // Bit width of the maximal delta x in the
                                 // header.
        int_vector<> m_lcp;      // Array for the relative bit-Lcp values.
        int_vector<> m_sa;       // Array for the suffix array values.
        bit_vector m_bp_ct; // Bitvector for the balanced parentheses of the
                            // Cartesian Tree of the LCP Array
    public:
        ...
        void set_header(const std::vector<header_item>& vh)
        {
            ...
            // as vh is asserted to be sorted by bwd_id we can take the last
            // element
            ...
            uint8_t total_width = m_width_bwd_id + m_width_delta_x
                                  + width_delta_d;
            if (total_width > 64) {
                std::cerr << "ERROR: header triple is to big for a 64bit "
                             "integer / its " << total_width << " bits "
                          << std::endl;
            }
            ...
            for (size_t i = 0; i < vh.size(); ++i) {
                ...
                m_header[i]
                    = m_header[i]
                      | (vh[i].delta_d << (m_width_bwd_id + m_width_delta_x));
            }
        }
         ...
         * \pre rb>lb
         */
        inline void set_content(const int_vector<64>& sa_buf,
                                const int_vector<64>& lcp_buf,
                                const unsigned char* text, size_type block_len)
        {
            size_type max_sa = 0;
            size_type max_lcp = 0;
            m_lcp = int_vector<>(block_len);
            m_sa = int_vector<>(block_len);
            for (size_type i = 1; i <= block_len;
                 ++i) { // copy SA and LCP values of the block
                m_sa[i - 1] = sa_buf[i];
            }
            ...
            /*
                                if (util::verbose) {
                                    ...
                                    std::cout << "LCP ="; for (size_type i=0;
               i<m_lcp.size(); ++i) {
                                        std::cout << " " << m_lcp[i] << "("<<
               lcp_buf[i+1] <<") ";
                                    }; std::cout << std::endl;
        ...
        void calculate_bp(bit_vector& bp_ct, const rac& lcp)
        {
            ...
            for (size_type i = 0, idx = 0, x; i < N; ++i) {
                while (!prev.empty() and(x = lcp[prev.top()]) > lcp[i]) {
                    prev.pop();
                    idx++; // move forward in bp_ct == write a closing
                           // parenthesis
                }
        ...
        template <class rac>
        void calculate_bit_lcp(rac& lcp, const int_vector<64>& sa_buf,
                               const int_vector<64>& lcp_buf,
        ...
        template <class rac>
        void calculate_relative_bit_lcp(rac& lcp, size_type lb, size_type rb,
                                        size_type min_lcp)
        {
            if (lb < rb) {
                ...
                calculate_relative_bit_lcp(lcp, lb, min_idx,
                                           lcp[min_idx]); // left sub-array
         ...
         */
        void decode_header_triple(const size_type i, size_type& bwd_id,
                                  size_type& delta_x, size_type& delta_d) const
        {
         ...
         *  \param delta_d  Output reference for delta_d.
         */
        // TODO: using binary search may be and optimisation if header.size() is
        // big
        void get_delta_x_and_d(const size_type bwd_id, size_type& delta_x,
                               size_type& delta_d)
        {
            for (size_type i = 0; i < m_header.size(); ++i) {
                if ((m_header[i] & bit_magic::Li1Mask[m_width_bwd_id])
                    == bwd_id) {
                    uint64_t w = m_header[i] >> m_width_bwd_id;
                    delta_x = w & bit_magic::Li1Mask[m_width_delta_x];
                    w >>= m_width_delta_x;
                            ...
                            std::string name = "") const
        {
            ...
            written_bytes += util::write_member(m_width_bwd_id, out, child,
                                                "width_bwd_id");
            written_bytes += util::write_member(m_width_delta_x, out, child,
                                                "width_delta_x");
            written_bytes += m_header.serialize(out, child, "header");
            written_bytes += m_bp_ct.serialize(out, child, "bp_ct");
            written_bytes
                += LcpSerializeWrapper(m_lcp).serialize(out, child, "lcp");
    ...
    void open_streams()
    {
        if (m_fac_dens == 0) {
            ...
            open_stream(m_glz_text, get_factorization_filename().c_str());
            {
                ...
                if (util::verbose) {
                    cout << "m_lz_width = " << (int)m_lz_width
                         << " m_lz_size = " << m_lz_size << endl;
                }
            }
        }
    ...
    void open_stream(ifstream& stream, const char* file_name)
    {
        ...
        if (!stream) {
            std::cerr << "Notice: Could not open file: " << file_name
                      << std::endl;
        } else {
...
public:
    ...
    {
        return get_output_dir(file_name, output_dir) + "/"
               + util::basename(file_name) + "." + util::to_string(b) + "."
               + util::to_string(fac_dens) + ".2." + SDSL_XSTR(LCP_WRAP)
               + ".bt2.ext_idx";
    }
    ...
    {
        return get_ext_idx_filename(m_file_name.c_str(), m_b, m_fac_dens,
                                    m_output_dir.c_str());
    }
    ...
    {
        return get_ext_idx_filename(m_file_name.c_str(), m_b, m_fac_dens,
                                    m_output_dir.c_str()) + ".tmp";
    }


    //! Get the name of the file where the factorization of the index is stored.
    static string get_factorization_filename(const char* file_name, size_type b,
                                             const char* output_dir = NULL)
    {
        return get_output_dir(file_name, output_dir) + "/"
               + util::basename(file_name) + "." + util::to_string(b)
    ...
    {
        return get_factorization_filename(m_file_name.c_str(), m_b,
                                          m_output_dir.c_str());
    }

    //! Get the name of the file where the factorization of the index is stored.
    static string get_factor_border_filename(const char* file_name, size_type b,
                                             const char* output_dir = NULL)
    {
        return get_output_dir(file_name, output_dir) + "/"
               + util::basename(file_name) + "." + util::to_string(b)
    ...
    {
        return get_factor_border_filename(m_file_name.c_str(), m_b,
                                          m_output_dir.c_str());
    }
    ...
    {
        string path = get_output_dir(file_name, output_dir) + "/"
                      + util::basename(file_name);
        std::remove((path + "." + TMP_FWD_CSA_SUFFIX).c_str());
        std::remove((path + "." + TMP_BWD_CSA_SUFFIX).c_str());
    }

    /*!
     *  \param cst          CST which should be loaded or
     *constructed.
     *  \param file_name    Location of the CST on disk.
     *  \param tmp_dir      Location of the temporary directory for the
     *construction.
     *  \param delete_tmp   Boolean flag, if the generated files uncompressed
     *SA, LCP, and so on should be deleted.
     */
    void construct_or_load_fwd_csa(tCsa& csa, string file_name, string tmp_dir,
                                   bool delete_tmp)
    {
     ...
     *SA, LCP, and so on should be deleted.
     */
    void construct_or_load_bwd_csa(tCsa& bwd_csa, string file_name,
                                   string tmp_dir, bool delete_tmp)
    {
        ...
        if (!tmp_bwd_csa_stream) {
            ...
            if (!rev_text_stream) {
                ...
                for (size_type i = 0, j = text.size() - 1;
            ...
            if (util::verbose) {
                std::cout << "load stored bwd_csa form file " << file_name
                          << std::endl;
            }
    ...
    void calculate_bl_and_bf_and_mapping(const tCsa& csa,
                                         vector<block_info>& map_info)
    {
        bit_vector bl = bit_vector(csa.size());
        bit_vector bf
            = bit_vector(csa.size() + 1); // add one bit for technical reasons
        tQPII q;
        q.push(tPII(0, csa.size())); // init queue with root interval
                                     // [0..csa.size()]
        tQI q_fp;
        q_fp.push(0); // init queue for the forward position of the left bound
                      // of the interval
        tQI q_depth;
        q_depth.push(0); // init queue for depth of the interval
        size_type k;     // number of different symbols in interval [lb..rb]
        vector<unsigned char> cs(
            256); // cs[0..k-1] contains the different symbols of [lb..rb]
        vector<size_type> rank_lb(256), rank_rb(256); // rank_lb[i] = rank(lb,
                                                      // cs[i]) and rank_rb[i] =
                                                      // rank(rb, cs[i])
        ...
        while (!q.empty()) {
            ...
            if (rb - lb > m_b) { // if
                ...
                csa.wavelet_tree.interval_symbols(lb, rb, k, cs, rank_lb,
                                                  rank_rb);
                // sort the different symbols lexicographically
                tPQPCI pq; // by using a priority queue
                for (size_type i = 0; i < k;
                     ++i) { // push character and its index to pq to sort it
                    pq.push(tPCI(cs[i], i)); // this step would be not necessary
                                             // if we use a sorted WT
                }
                for (size_type i = 0, j, lex_smaller = 0; i < k; ++i) {
                    j = pq.top().second;
                    pq.pop();
                    size_type c_begin = csa.C[csa.char2comp[cs[j]]];
                    size_type lb_new = c_begin + rank_lb[j],
                              rb_new = c_begin + rank_rb[j];
        ...
        util::assign(m_bl, bl);
        if (util::verbose) std::cout << "m_bl was assigned.\n";
        util::assign(m_bf, bf);
        if (util::verbose) std::cout << "m_bf was assigned.\n";
        util::init_support(m_bf_rank, &m_bf);
        if (util::verbose) std::cout << "m_bf_rank was assigned.\n";
        util::init_support(m_bf_select, &m_bf);
        if (util::verbose) std::cout << "m_bf_select was assigned.\n";
        util::init_support(m_bl_select, &m_bl);
        if (util::verbose) std::cout << "m_bl_select was assigned.\n";
    }

    void calculate_bm_and_min_depth(const vector<block_info>& map_info,
                                    const size_type f_k)
    {
        if (util::verbose) cout << "m_k=" << m_k << endl;
        if (util::verbose) cout << "f_k=" << f_k << endl;
        if (util::verbose)
            cout << "map_info.size()=" << map_info.size() << endl;
        ...
        for (size_type i = 1, j = 0, p = 0, end = m_bf_rank(m_bf.size());
             i < end; ++i) {
        ...
        util::init_support(m_bm_1_select, &m_bm);
        if (util::verbose) cout << "m_bm_1_select was assigned.\n";
        util::init_support(m_bm_0_select, &m_bm);
        if (util::verbose) cout << "m_bm_0_select was assigned.\n";
        util::init_support(m_bm_10_rank, &m_bm);
        if (util::verbose) cout << "m_bm_10_rank was assigned.\n";
        ...
        for (size_type i = 1, j = 0, k = 0, end = m_bf_rank(m_bf.size());
             i < end; ++i) {
            size_type lb = m_bf_select(i);
    ...
    void calculate_bwd_id_and_fill_singleton_pointers(
        const char* sa_file, const tCsa& csa,
        const vector<block_info>& map_info, const bit_vector& fwd_bf,
        vector<block_node>& v_block, bit_vector& is_singleton)
    {
        rank_support_v5<> fwd_bf_rank(&fwd_bf);
        int_vector<> block_sa(fwd_bf_rank(fwd_bf.size()), 0,
                              bit_magic::l1BP(fwd_bf.size()) + 1);
        int_vector_file_buffer<> sa_buf(sa_file);
        for (size_type i = 0, r = 0, r_sum = 0, idx = 0;
             i < sa_buf.int_vector_size;) {
            for (; i < r + r_sum; ++i) {
                if (fwd_bf[i]) {
        ...
        for (size_t bwd_id = 0; bwd_id < map_info.size(); ++bwd_id) {
            ...
            size_type fwd_id = fwd_bf_rank(fwd_lb + 1)
                               - 1; // calculate corresponding forward id
        ...
        if (util::verbose)
            cout << "SA pointer for singleton block were inserted.\n";
        if (util::verbose)
            cout << "singleton block were marked in bit_vector is_singleton.\n";
        if (util::verbose)
    ...
    uint64_t get_next_int(ifstream& stream, uint8_t width, uint8_t& offset,
                          uint64_t& cached_word)
    {
        uint64_t res = 0;
        if (offset == 0) {
            ...
            stream.read((char*)&cached_word, sizeof(uint64_t));
            offset = (offset + width) & 0x3F;
            res |= (((cached_word) & bit_magic::Li1Mask[offset])
                    << bit_of_old_word);
        }
        return res;
    }
     ...
     */
    void write_external_blocks_and_pointers(
        const char* sa_file, const char* lcp_file, const bit_vector& fwd_bf,
        ...
        vector<vector<header_item>>& header_of_external_block)
    {
        ...
        if (ext_idx_out) {
            // TODO: test if the size of the output buffer chances the
            // performance
            ...
            uint64_t sa_size, lcp_size, sa_word = 0, lcp_word = 0, sa_idx = 0,
                                        lcp_idx = 0;
            uint8_t sa_width, lcp_width, sa_off = 0, lcp_off = 0;
            sa_stream.read((char*)&sa_size, sizeof(sa_size));
            lcp_stream.read((char*)&lcp_size, sizeof(lcp_size));
            sa_stream.read((char*)&sa_width, sizeof(sa_width));
            lcp_stream.read((char*)&lcp_width, sizeof(lcp_width));
            int_vector<64> sa_buf(m_b + 1, 0);
            int_vector<64> lcp_buf(m_b + 1, 0);
            size_type sa_size_in_bytes1 = 0;

            for (size_t fwd_id = 0; fwd_id < header_of_external_block.size();
                 ++fwd_id) {
                if (header_of_external_block[fwd_id].size()
                    > 0) { // if it is an irreducible block
                    sort(header_of_external_block[fwd_id].begin(),
                         header_of_external_block[fwd_id].end());
                    disk_block db;
                    db.set_header(header_of_external_block[fwd_id]);
                    size_type lb = fwd_bf_select(fwd_id + 1),
                              rb = fwd_bf_select(fwd_id + 2) - 1;
                    if (lb == 0) {
                        throw std::logic_error(
                            "lb==0: No sentinel character appended?");
                    }
                    while (sa_idx < lb - 1) {
                    ...
                    for (size_type i = (sa_idx == lb); sa_idx <= rb;
                         ++i, ++sa_idx, ++lcp_idx) {
                        sa_buf[i] = get_next_int(sa_stream, sa_width, sa_off,
                                                 sa_word);
                    ...
                    db.set_content(sa_buf, lcp_buf, (const unsigned char*)text,
                                   block_len);
            ...
            delete[] text;
            if (util::verbose)
                cout << "# ext_idx_size_in_MB = "
                     << ext_idx_size_in_bytes / (1024.0 * 1024.0) << "\n";
            if (util::verbose)
                cout << "# ext_idx_header_size_in_MB = "
                     << total_header_in_bytes / (1024.0 * 1024) << "\n";
            if (util::verbose)
                cout << "# sa_size_in_MB = " << ((double)sa_size_in_bytes1)
                                                / (1024 * 1024.0) << "\n";
            if (util::verbose)
                cout << "# sa_size1_in_MB = " << ((double)sa_size_in_bytes1)
                                                 / (1024 * 1024.0) << "\n";
    ...
    {
        string tmp_file_name = tmp_dir + util::basename(m_file_name) + "_cBWT_"
                               + util::to_string(util::get_pid()) + "_"
                               + util::to_string(util::get_id());
    ...
    void output_tikz()
    {
        ...
        string base_name = util::basename(file_name) + "."
                           + util::to_string(m_b);
        ...
        for (size_type bwd_id = 0; bwd_id < max_bwd_id; ++bwd_id) {
            fwd_blocks_in_bwd.push_back(
                bwd_id_to_info(bwd_id, bf_rank, fwd_cst));
        }
        write_tikz_output_bwd(bwd_out, bwd_csa, m_bf, m_bl, m_bm, m_b,
                              fwd_blocks_in_bwd, m_min_depth);


        ofstream factor_out((base_name + ".fac.tex").c_str());

        int_vector<> factorization;
        util::load_from_file(factorization,
        ...
        for (int i = tmp.size() - 2; i >= 0; --i)
            text[tmp.size() - 2 - i]
                = util::to_latex_string((unsigned char)tmp[i]);
        text[tmp.size() - 1]
            = util::to_latex_string((unsigned char)tmp[tmp.size() - 1]);
        ...
        sort(map_info.begin(), map_info.end()); // sort according to bwd_lb,
                                                // depth, fwd_lb and size
        vector<block_node> v_block; // block_node contains (delta_x, delta_d,
                                    // dest_block, bwd_id)
        size_type red_blocks = 0;
        size_type singleton_blocks = 0;
        size_type elements_in_irred_blocks = 0;
        calculate_reducible_graph(fwd_cst.csa, bf, m_b, red_blocks,
                                  singleton_blocks, elements_in_irred_blocks,
                                  v_block);
        for (size_t i = 0; i < fwd_blocks_in_bwd.size(); ++i) {
            v_block[fwd_blocks_in_bwd[i].block_id].bwd_id = i;
        }
        vector<vector<header_item>> header_of_external_block(m_k);
        calculate_headers(v_block, map_info, header_of_external_block);

        write_tikz_output_fwd(fwd_out, fwd_cst, bf, m_b, v_block,
                              header_of_external_block);
    }


    //! Constructor
    /*!
     *  \param file_name        File name of the file which contains the
     *text.
     *  \param b                Threshold parameter b for the maximum size of a
     *block.
     ...
     *  \param delete_tmp       Indicates if the temporary files that are used
     *during the construction should be removed after the construction.
     ...
     */
    rosa(const char* file_name = NULL, size_type b = 4096,
...
#endif
    {
        ...
        m_buf_lz = new uint64_t[m_buf_size + (m_fac_dens - 1)]; // initialise
                                                                // buffer for
                                                                // pattern
        if (NULL == file_name) {
        ...
        //          (0) Initialise path for the external parts of the data
        // structure
        ...
        string tmp_dir = (util::dirname(tmp_file_dir) + "/"
                          + util::basename(tmp_file_dir) + "/");


        if (m_fac_dens > 0) {
            string zero_ext_idx_file = get_ext_idx_filename(
                m_file_name.c_str(), b, 0, m_output_dir.c_str());
            ifstream zero_ext_stream(zero_ext_idx_file.c_str());
            if (zero_ext_stream) {
                std::cout << "CACHE: 0-external index existed at "
                          << zero_ext_idx_file << "; opened stream;"
                          << std::endl;

                string zero_int_idx_file = get_int_idx_filename(
                    m_file_name.c_str(), b, 0, m_output_dir.c_str());
                ifstream zero_int_stream(zero_int_idx_file.c_str());
                if (zero_int_stream) {
                    std::cout << "CACHE: 0-internal index existed at "
                              << zero_int_idx_file << "; opened stream;"
                              << std::endl;
                    ...
                    // scan trough blocks and if a bwd_id occurs in the header
                    // mark is_singleton[bwd_id]=0!
                    ...
                    write_R_output("CACHE: ext_idx", "replace_pointers",
                                   "begin");
        ...
        construct_or_load_fwd_csa(fwd_csa, fwd_csa_file_name, tmp_dir,
                                  delete_tmp);
        ...
        write_R_output("lcp", "construct", "begin");
        {
            ...
            if (!lcp_in) { // if the lcp file does not exist: construct it
                ...
                config.file_map[constants::KEY_TEXT]
                    = util::cache_file_name(constants::KEY_TEXT, config);
        ...
        /*
           ...
           algorithm::extract(fwd_cst.csa, 0, fwd_cst.csa.size()-1);
                                ofstream
           check_out((util::basename(file_name)+".check_text").c_str());
                                check_out.write(check_text.c_str(),
           check_text.size());
                                check_out.close();
        */
        ...
        m_n = fwd_csa.size();
        if (util::verbose) cout << "m_n=" << m_n << endl;
        ...
        util::clear(fwd_csa);
        if (util::verbose) cout << "cleared fwd_csa" << endl;
        //          (3) Load or construct the backward CSA
        tCsa bwd_csa;
        write_R_output("bwd_csa", "construct", "begin");
        construct_or_load_bwd_csa(bwd_csa, bwd_csa_file_name, tmp_dir,
                                  delete_tmp);
        ...
        util::clear(bwd_csa);
        if (util::verbose) cout << "cleared bwd_csa" << endl;
        ...
        calculate_reducible_graph(fwd_csa, fwd_bf, m_b, red_blocks,
                                  singleton_blocks, elements_in_irred_blocks,
                                  v_block);
        write_R_output("reducible graph", "construct", "end");
        // now the entries delta_x, delta_d and dest_block are known for each
        // block in forward order.
        ...
        calculate_bwd_id_and_fill_singleton_pointers(
            sa_file.c_str(), fwd_csa, map_info, fwd_bf, v_block, is_singleton);
        ...
        write_external_blocks_and_pointers(sa_file.c_str(), lcp_file.c_str(),
                                           fwd_bf, v_block,
                                           header_of_external_block);
            ...
            for (size_type i = 0, r = 0, r_sum = 0;
                ...
                for (; i < r + r_sum; ++i) {
                    ...
                    if (i + 1 == glz_buf.int_vector_size) {
                        factor_string
                            = factor_string.substr(0, factor_string.size() - 1);
                    }
    ...
    void factor_frequency()
    {
        if (m_fac_dens > 0) {
            string out_name
                = ("./" + util::basename(m_file_name) + ".occ_freq");
            ofstream res_out(out_name.c_str());
    ...
    void fill_singleton_bit_vector(std::ifstream& ext_idx_stream,
                                   bit_vector& is_singleton)
    {
        ...
        while (ext_idx_stream.tellg() < end) {
            ...
            for (size_type i = 0; i < db.header.size();
                 ++i) { // loop through header entries
                size_type bwd_id, delta_x, delta_d;
                db.decode_header_triple(i, bwd_id, delta_x, delta_d);
                is_singleton[bwd_id]
                    = 0; // delete singleton flag for non-singleton blocks
            }
        }
    }
     ...
     */
    void replace_pointers(const bit_vector& factor_borders,
                          const bit_vector& is_singleton, string stream_name)
    {
        ...
        map<uint64_t, uint64_t> old_pointer_to_new; // maps the old pointers to
                                                    // the new ones
     ...
     *  \return The backward id of the block (in [0..k-1], where k is the
    ...
    /*!
     */
    size_type locate(const unsigned char* pattern, size_type m,
                     vector<size_type>& res) const
    {
        res.resize(0);
        ...
        if (get_interval(pattern, m, lb, rb, d)) {
            if (d == m) { // the in-memory data structure answered the query
                // we have to get all blocks between lb and rb and output the
                // positions
                if (util::verbose) {
                ...
                if (rb + 1 - lb == 1) {
                    ...
                    if (m_fac_dens
                        > 0 and match_pattern_lz(pattern, m, m_fac_dens * sa)) {
                        res.push_back(sa);
                        replace_pointers_by_positions(pattern, m, res);
                        return 1;
                    } else if (0 == m_fac_dens and match_pattern(
                                        pattern + d, m - d, sa + d)) {
                        res.push_back(sa);
                        return 1;
                    }
                    ...
                    size_type count
                        = search_block(pattern + d, m - d, d, rb + 1 - lb,
                                       bwd_id, block_addr, &res);
                    if (m_fac_dens > 0) {
                        replace_pointers_by_positions(pattern, m, res,
                                                      block_addr);
                    }
     ...
     *  \param
     *  \return         The number of occurrences of the pattern in the
     * text.
     */
    size_type count(const unsigned char* pattern, size_type m,
                    bool repeated_in_memory_search = false) const
    {
        m_matchlz_performed = false;
        if (util::verbose) {
        ...
        if (get_interval(pattern, m, lb, rb, d)) {
            if (d == m) { // the in-memory data structure answered the query
                ...
                // (1a) try if the pattern P[d..m-1] and so on also appear in
                // the text
                if (repeated_in_memory_search) {
                    ...
                    while (m > td) {
                        size_type delta_d = 0;
                        if (get_interval(shorter_pattern, m - td, tlb, trb,
                                         delta_d)) {
                            // delta_d should be > 0, since m-td > 0 and
                            // get_interval only returns true, if something is
                            // matched
                            td += delta_d;
                ...
                if (rb + 1 - lb == 1) {
                    ...
                    if ((m_fac_dens
                         > 0 and match_pattern_lz(pattern, m, m_fac_dens * sa))
                        or(0 == m_fac_dens and
                                    match_pattern(pattern + d, m - d, sa + d)))
                        return 1;
                } else {
                    size_type block_addr = m_pointer[bwd_id];
                    return search_block(pattern + d, m - d, d, rb + 1 - lb,
                                        bwd_id, block_addr);
                }
            }
        }
     ...
     */
    bool get_interval(const unsigned char* pattern, size_type m, size_type& lb,
                      size_type& rb, size_type& d) const
    {
        d = 0;
        lb = 0;
        rb = m_n - 1;
        if (util::verbose) {
            std::cout << "pattern=" << pattern << "," << m << "," << lb << ","
                      << rb << "," << d << "\n";
        }
        ...
        while (d<m and rb - lb + 1> m_b) {
            if (util::verbose) {
                std::cout << d << "-[" << lb << "," << rb << "]"
                          << " b=" << m_b << std::endl;
                std::cout << "c=" << *cp << " m_cC[m_char2comp[" << *cp
                          << "]]=" << m_cC[m_char2comp[*cp]] << std::endl;
            }
                                  ...
                                  const cst_sct3<>& cst)
    {
        ...
        for (size_type i = 0, _lb = lb; i < depth; ++i) {
            ...
            factor.push(c);
            size_type c_rank = m_bf_rank(_lb) + m_bf[_lb]
                               - m_cC[m_char2comp[c]];
            size_type cpos = m_wt.select(c_rank, c);
        ...
        algorithm::backward_search(cst.csa, (size_type)0, cst.csa.size() - 1,
                                   (const unsigned char*)str.c_str(),
                                   str.size(), fwd_lb, fwd_rb);
    ...
    unsigned char first_row_character(size_type i) const
    {
        ...
        if (m_wt.sigma < 16) {
            ...
            size_type upper_c = m_wt.sigma,
                      lower_c = 0; // lower_c inclusive, uppper_c exclusive
            size_type res = 0;
            do {
    ...
    /*!
     * Parse the text greedily into factors. Each factor (except the last one)
     *corresponds to
     * a block prefix of an external block.
     *
     * \par Working set size
     *      Constant size buffer for the text + constant size buffer for the
     *output + n bits for factor borders
     */
    size_type greedy_parse(string tmp_dir, bit_vector& factor_borders,
                           size_type fac_dens)
    {
        if (m_b >= m_n) {
            throw std::logic_error("greedy_parse: m_b=" + util::to_string(m_b)
                                   + " >= " + util::to_string(m_b) + "=m_n");
        }
        ...
        if (factor_border_stream) {
            ...
            if (test_factor_stream) {
                ...
                std::cout << "greedy_parse: factor width = " << (int)m_lz_width
                          << std::endl;
                std::cout
                    << "greedy_parse: factor border file exists; load it from "
                    << get_factor_border_filename() << std::endl;
                util::load_from_file(factor_borders,
                                     get_factor_border_filename().c_str());
        ...
        int_vector_file_buffer<8> text_buf(
            util::cache_file_name(constants::KEY_TEXT, config).c_str());
        // file name for temporary
        string factor_file = tmp_dir + util::basename(m_file_name) + "_factors_"
                             + util::to_string(util::get_pid()) + "_"
                             + util::to_string(util::get_id());
        ...
        if (factor_stream) {
            util::assign(factor_borders,
                         bit_vector(text_buf.int_vector_size, 0));
            size_type factors = 0;
            size_type lb = 0, rb = m_n - 1, d = 0;

            size_type dont_sample_it = fac_dens;
            for (size_type i = 0, r = 0, r_sum = 0;
                 i < text_buf.int_vector_size;) {
                for (; i < r + r_sum; ++i) {
                    size_type lb1 = m_bl_rank(lb);
                    size_type rb1 = m_bl_rank(rb + 1);
                    ...
                    size_type rb2 = m_wt.rank(rb1, c);
                    if (lb2 == rb2) {
                        throw std::logic_error("greedy_parse: parse not "
                                               "possible with block prefixes");
                    } else {
            ...
            factor_stream.close();
            int_vector<> factorization;
            util::load_vector_from_file(factorization, factor_file.c_str(),
                                        num_bytes);
            std::remove(factor_file.c_str()); // remove temp file
            util::bit_compress(factorization);
            m_lz_width = factorization.get_int_width();
            util::store_to_file(factorization,
                                get_factorization_filename().c_str());
            if (fac_dens == 1) {
                util::store_to_file(factor_borders,
                                    get_factor_border_filename().c_str());
            }
            return factors;
        } else {
            throw std::logic_error(
                ("Greedy parse: Could not open temporary file: " + factor_file)
                    .c_str());
            return 0;
        }
    }
     ...
     * \param lb         Left bound of the lexicographic interval of the already
     *matched prefix.
     ...
     * \param locations  Pointer to a vector, where the locations should be
     *stored.
     * \param
     *
     * \par Time complexity
     *      2 disk accesses
     */
    size_type get_all_occurences(size_type d, size_type lb, size_type rb,
                                 std::vector<size_type>& loc) const
    {
        loc.resize(rb + 1 - lb);
        size_t loc_idx = 0;
        ...
        while (!q.empty()) {
            ...
            if (x.rb - x.lb + 1 > m_b) {
                ...
                if (x.rb + 1 - x.lb == 1) {
                    ...
                    // push triple
                    res_block.push_back(
                        block_item(m_pointer[bwd_id], bwd_id, x.rb + 1 - x.lb));
        ...
        sort(res_block.begin(),
             res_block.end()); // sort according to block_addr
        disk_block db;
        for (size_t i = 0; i < res_block.size(); ++i) {
            if (0 == i or res_block[i - 1].block_addr
                != res_block[i].block_addr) {
                if (util::verbose) {
                    cout << "i=" << i << " res_block[i].block_addr="
                         << res_block[i].block_addr << endl;
                }
                seekg(m_ext_idx, res_block[i].block_addr);
                db.load(m_ext_idx);
                if (util::verbose) {
                    cout << "i=" << i
                         << " block_loaded loc.size()=" << loc.size()
                         << " loc_idx=" << loc_idx << endl;
                }
            }
    ...
    /*!
     *  The half-blind suffix tree is constructed from a rosa disk-block in
     * linear
    ...
    private:
         ...
         *   \f$ \Order{1} \f$
         */
        size_type get_first_l_index(const node_type& node,
                                    const bp_support_sada<>& bp_support,
                                    size_type& kpos, size_type& ckpos) const
        {
            if (node.cipos
                > node.jp1pos) { // corresponds to m_lcp[i] <= m_lcp[j+1]
                ckpos = node.jp1pos - 1;
        ...
        // Get the previous smaller value. Adapted for the case that there are
        // no equal values in the array.
        /*
         * \par Time complexity
         *    \f$ \Order{\frac{\sigma}{w}} \f$, where w=64 is the word size, can
         * be implemented in \f$\Order{1}\f$ with rank and select
         */
        inline size_type psv(size_type i, size_type ipos, size_type cipos,
                             size_type& psvpos, size_type& psvcpos,
                             const bp_support_sada<>& bp_support) const
        {
         ...
         *       \f$ \Order{1}\f$
         */
        node_type parent(const node_type& v, const bit_vector bp,
                         const bp_support_sada<>& bp_support) const
        {
            if (v.cipos > v.jp1pos) { // LCP[i] <= LCP[j+1]
                size_type psv_pos, psv_cpos, psv_v, nsv_v, nsv_p1pos;
                psv_v = psv(v.j + 1, v.jp1pos, bp_support.find_close(v.jp1pos),
                            psv_pos, psv_cpos, bp_support);
                ...
                psv_v
                    = psv(v.i, v.ipos, v.cipos, psv_pos, psv_cpos, bp_support);
    ...
    public:
         ...
         *  \param depth    Number of characters already matched (in
         *characters).
         *  \param lb       Left boundary of the search interval
         *(inclusive).
         *  \param rb       Right boundary of the search interval
         *(inclusive).
         *  \return The number of potential occurrences of the pattern in
         *the block.
         *
         ...
         */
        size_type get_interval(const unsigned char* pattern, size_type m,
                               size_type depth, size_type& lb, size_type& rb)
        {
            // search process which can lead to a false positive result
            size_type lb_pos = m_bp_ct_support.select(lb + 1);
            size_type rb_p1_pos
                = (rb + 1 < size() ? m_bp_ct_support.select(rb + 2) : 2
                                                                      * size());
            node_type v(lb, rb, lb_pos, m_bp_ct_support.find_close(lb_pos),
                        rb_p1_pos);
            if (is_leaf(v)) {
                return 1;
            }
            ...
            while (!is_root(w)) {
                ...
                if (util::verbose) {
                    std::cout << w.i << " " << w.j
                              << " v_bit_depth=" << v_bit_depth << std::endl;
                }
            ...
            if (util::verbose) {
                std::cout << w.i << " " << w.j << " v_bit_depth=" << v_bit_depth
                          << std::endl;
            }
            ...
            while (1) {
                ...
                size_type l
                    = get_first_l_index(v, m_bp_ct_support, lpos, clpos);
     ...
     *      2 disk accesses
     */
    size_type search_block(const unsigned char* pattern, size_type m,
                           size_type depth, size_type size, size_type bwd_id,
                           size_type block_addr,
                           std::vector<size_type>* loc = NULL) const
    {
        if (util::verbose) {
            std::cout << "serach_block(" << pattern << "," << m << "," << depth
                      << "," << size << "," << bwd_id << "," << block_addr
                      << ")\n";
        }
        ...
        seekg(m_ext_idx,
              block_addr);  // seek to the start address of the disk block
        ...
        size_type lb = delta_x,
                  rb = delta_x + size - 1; // determine left and right bound
        ...
        // don't to the actual matching but something; but use the block_tree,
        // so that
        // the construction is no optimized out
        size = std::min(size, tree.size());
#else
        size = tree.get_interval(pattern, m, depth + delta_d, lb,
                                 rb); // do the matching
...
#if defined BENCHMARK_LOAD_ONLY || defined BENCHMARK_CREATE_ONLY               \
    || defined BENCHMARK_SEARCH_BLOCK_ONLY
        return size;
#endif
        if (size
            > 0 and((m_fac_dens
                     > 0 and match_pattern_lz(pattern - depth, m + depth,
                                              m_fac_dens * db.sa[lb]))
                    or(0 == m_fac_dens and match_pattern(
                                pattern, m, db.sa[lb] + depth + delta_d)))) {
            if (NULL != loc) {
                if (m_fac_dens > 0) {
     ...
     */
    bool match_pattern(const unsigned char* pattern, size_type m,
                       size_type text_offset) const
    {
        ...
        if (text_offset + m
            > m_n - 1) { // m_n-1 text size without the sentinel character
            return false;
        }
    ...
    void calculate_kmp_table(const unsigned char* pattern, size_type m,
                             int_vector<>& kmp_table) const
    {
                                       ...
                                       size_type block_addr = 0) const
    {
        ...
        for (size_type fs = 0; fs < factor_sample_pointer.size();) {
            ...
            // cout<<"m_factor_border_select("<<factor_sample_pointer[fs-occs]<<")="<<
            // m_factor_border_select(factor_sample_pointer[fs-occs])<<endl;
            // TODO add m_factor_border m_factor_border_select
            size_type position = 0;
            if (factor_sample_pointer[fs - occs] > 0) {
                position
                    = m_factor_border_select(factor_sample_pointer[fs - occs])
                      + 1; // map starting factor to text position
            }
            ...
            if (util::verbose)
                cout << "factor_nr = " << factor_nr
                     << "  /  lz_size = " << m_lz_size << endl;
            if (util::verbose)
                cout << "factor_bit_offset = " << factor_bit_offset << endl;

            seekg(m_glz_text, ((factor_bit_offset) / 64) * 8 + 9);

            size_type len
                = ((m_lz_width * (factor_nr + m + (m_fac_dens - 1)) - 1) / 64)
                  - ((m_lz_width * factor_nr) / 64) + 1; // len in 64 bit words
            if (len > m_buf_size + (m_fac_dens - 1)) {
                cout << "WARNING (replace pointers by positions): buffer to "
                        "small" << endl;
            ...
            if (cur_end_word
                > the_end_word) { // check that we don't go beyond EOF
            ...
            if (util::verbose) {
                cout << "XXX m_glz_text.fail()=" << m_glz_text.fail() << endl;
            ...
            while (
                occs and(word < m_buf_lz + len
                                - 1 or(word == m_buf_lz + len - 1 and bit_offset
                                               + m_lz_width <= 64))) {
                uint64_t factor_id = bit_magic::read_int_and_move(
                    word, bit_offset, m_lz_width);
                if (util::verbose)
                    cout << "factor id=" << factor_id << " word=" << word
                         << " bit_offset=" << (int)bit_offset
                         << " m_lz_width=" << (int)m_lz_width
                         << " / end_word=" << m_buf_lz + len - 1
                         << " end offset=" << (int)bit_offset << endl;
                string factor_string;
                ...
                for (size_type i = 0; i < factor_string.size() and occs;) {
                    if (matched < m and(unsigned char)factor_string[i]
                        == pattern[matched]) {
                        if (matched == m - 1) {
                            --occs;
                ...
                if (util::verbose)
                    cout << "replace: matched so far = " << matched
                         << " occs=" << occs << " i=" << fs << "/"
                         << factor_sample_pointer.size()
                         << " position=" << position << endl;
                if (factor_id == 0) break;
            }
        }
    }
    ...
    {
        return ((double)util::get_file_size(get_ext_idx_filename().c_str()))
               / (1024.0 * 1024.0);
    }
                        ...
                        std::string name = "") const
    {
        ...
        written_bytes
            += util::write_member(m_file_name, out, child, "file_name");
        written_bytes
            += util::write_member(m_output_dir, out, child, "output_dir");
        written_bytes += util::write_member(m_fac_dens, out, child, "fac_dens");
        if (m_fac_dens > 0 and m_factor_border.size() > 0) {
            written_bytes
                += m_factor_border.serialize(out, child, "factor_border");
            written_bytes += m_factor_border_select.serialize(
                out, child, "factor_border_select");
        }
        structure_tree::add_size(child, written_bytes);
        return written_bytes;
    }
    ...
    void load(std::istream& in)
    {
        ...
        if (m_fac_dens > 0 and in.eof()) {
            ...
            if (util::load_from_file(factor_border,
                                     get_factor_border_filename().c_str())) {
                for (size_type i = 0, do_not_sample = m_fac_dens;
                     i < factor_border.size(); ++i) {
                    if (factor_border[i]) {
                        if (--do_not_sample) {
                            factor_border[i] = 0;
                        } else {
    ...
    void statistics() const
    {
        ...
        while (m_ext_idx.tellg() < end) {
            ...
            k_re += (db.header.size()
                     - 1); // increase the number of reducible blocks
            n_ir += db.sa.size();

            header_in_megabyte += util::get_size_in_mega_bytes(db.header);
            lcp_in_megabyte
                += util::get_size_in_mega_bytes(LcpSerializeWrapper(db.lcp));
        ...
        std::cout << "# header_in_megabyte = " << header_in_megabyte
                  << std::endl;
        ...
        if (m_fac_dens == 0) {
            std::cout << util::get_file_size(m_file_name.c_str())
                         / (1024.0 * 1024);
        } else {
            std::cout << util::get_file_size(
                             get_factorization_filename().c_str())
                         / (1024.0 * 1024.0);
        }
        std::cout << std::endl;
        double h0 = 0;
        for (map<uint64_t, uint64_t>::const_iterator it = lcp_map.begin();
             it != lcp_map.end(); ++it) {
     ...
     */
    bool match_pattern_lz(const unsigned char* pattern, size_type m,
                          size_type lz_offset) const
    {
        m_matchlz_performed = true;
    ...
    void read_factor_ids(sdsl::int_vector<>& buf,size_t lz_offset,size_t m) const {
        ...
        size_t len
            = ((m_lz_width * (lz_offset + m + (m_fac_dens - 1)) - 1) / 64)
              - ((m_lz_width * lz_offset) / 64) + 1; // len in 64 bit words

        size_t cur_end_word = (m_lz_width * lz_offset) / 64 + len;
        size_t the_end_word = (m_lz_width * m_lz_size + 63) / 64;
    ...
    decode_factor_id(size_t bwd_id) const { 
        ...
        size_t depth = m_min_depth[depth_idx];
        if (ones) {
            depth += zero_pos - 1 - m_bm_1_select(ones);
        } else {
            depth += zero_pos;
